...
const VehicleForm = memo(function VehicleForm({
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      {/* Image upload (kept as-is: file upload via API route) */}
      <div className="space-y-1.5 sm:space-y-2">
...
function VehicleManagementContent() {
  ...
  // Fade-in helper (for your existing CSS animations)
  useEffect(() => {
    const activate = () => {
  ...
  // Load vehicles from DB; UI-only fields are defaulted locally
  useEffect(() => {
    const load = async () => {
      if (!supabase) return
  ...
  // Keep secure upload via API route (no DB policy issues)
  ...
  const handleEditVehicle = useCallback(async (e: React.FormEvent) => {
    ...
    const updated: Vehicle = {
      ...
      description: formData.description || editingVehicle.description,
      features: formData.features ? formData.features.split(",").map(f => f.trim()).filter(Boolean) : editingVehicle.features,
    }
